...
def show_modal_troca(page: ft.Page, devolucao: dict, pdv_core, on_troca_callback):
    ...
    def confirmar_troca(e):
        ...
        print(f"[TROCA] Confirmando com: {valor}")

        if not valor:
            show_snackbar(page, "Digite um ID ou código de barras", COLORS["danger"])
            return
        ...
        try:
            ...
            if not existe:
                show_snackbar(page, "Produto não encontrado", COLORS["danger"])
                return

            novo_id = produto.id
            novo_nome = produto.nome
            quantidade = devolucao.get("quantidade", 1)
            estoque = produto.estoque_atual or 0

            if estoque < quantidade:
                show_snackbar(page, "Estoque insuficiente", COLORS["danger"])
                return

            adicionar_troca(devolucao.get("id"), novo_id, novo_nome)
            sucesso, msg = atualizar_estoque_troca(
                pdv_core, devolucao.get("produto_id"), novo_id, quantidade
            )

            if sucesso:
                show_snackbar(page, "✓ Troca realizada!", COLORS["success"])
                print("[TROCA] ✓ Sucesso")
            else:
                show_snackbar(page, f"Erro: {msg}", COLORS["warning"])
    ...
    print("[TROCA] Abrindo BottomSheet...")
